...
// 💡 1. 미슐랭 프로젝트 오렌지/레드 테마에 맞춰 포인트 컬러 세련되게 수정
const partConfig: Record<string, { label: string; color: string }> = {
  ALL:         { label: "전체",        color: "bg-purple-50 text-purple-700" },
  MEMBER:      { label: "회원관리",    color: "bg-teal-50 text-teal-700"     },
  RESTAURANT:  { label: "매장관리",    color: "bg-orange-50 text-orange-700" }, // blue -> orange
  REVIEW:      { label: "리뷰/댓글",   color: "bg-amber-50 text-amber-700"   },
  NOTICE:      { label: "공지사항",    color: "bg-green-50 text-green-700"   },
  POPUP:       { label: "팝업/광고",   color: "bg-red-50 text-red-700"       },
};
...
const AdminManagePage = () => {
  ...
  const [newPart, setNewPart] = useState("MEMBER");
    ...
    try {
      if (selectedAdmin.adminRole !== newRole) {
        const roleRes = await fetch(
          `${BASE_URL}/${selectedAdmin.adminId}/role?newRole=${newRole}`,
          {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
      ...
      if (selectedAdmin.adminPart !== newPart) {
        const partRes = await fetch(
          `${BASE_URL}/${selectedAdmin.adminId}/part?newPart=${newPart}`,
          {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
      ...
      <div className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            {/* 💡 디자인 포인트 바 색상 수정 (blue-700 -> orange-600) */}
          ...
          <div className="flex items-center gap-2">
            ...
            {/* 💡 버튼 디자인 수정 (blue -> orange) */}
            <button
              onClick={() => setIsModalOpen(true)}
              className="px-4 py-2 text-xs text-white bg-orange-600 hover:bg-orange-700 rounded-lg transition-colors"
            >
              + 관리자 등록
            </button>
          </div>
              ...
              {isLoading ? (
                ...
                sortedAdmins.map((admin) => {
                  ...
                  return (
                    <tr
                      key={admin.adminId}
                      className={`border-b border-gray-50 transition-colors ${isMe ? "bg-orange-50/30" : "hover:bg-gray-50"}`}
                    >
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          {isMe && (
                            <span className="px-1.5 py-0.5 text-xs bg-orange-100 text-orange-700 rounded-full font-medium">
                              나
                            </span>
                          )}
                        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white w-full max-w-sm mx-4 shadow-lg rounded-xl overflow-hidden">
            ...
            <div className="px-6 py-5 space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 border-b border-gray-300 focus:border-orange-600 outline-none py-2 text-sm text-gray-700 placeholder:text-gray-300"
                  />
                  <button
                    onClick={onSearchMember}
                    className="px-3 py-1 text-xs text-white bg-orange-600 hover:bg-orange-700 rounded-lg transition-colors"
                  >
                    검색
                  </button>
                </div>
                ...
                <div className="border border-gray-100 rounded-lg max-h-36 overflow-y-auto">
                  {searchResults.map((member) => (
                    <button
                      ...
                      className="w-full text-left px-3 py-2.5 text-sm hover:bg-orange-50 transition-colors border-b border-gray-50 last:border-0"
                    >
              ...
              {isSearching && <p className="text-xs text-gray-300 text-center py-2">검색 중...</p>}
              ...
              {selectedMember && (
                <div className="flex items-center justify-between bg-orange-50 px-3 py-2.5 rounded-lg">
                  <div>
                    <span className="text-sm text-orange-700 font-medium">{selectedMember.name}</span>
                    <span className="text-xs text-orange-400 ml-2">{selectedMember.email}</span>
                  </div>
                  <button
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full border-b border-gray-300 focus:border-orange-600 outline-none py-2 text-sm text-gray-700"
                >
              ...
              <div>
                ...
                >
                  ...
                  <option value="MEMBER">MEMBER (회원관리)</option>
                  <option value="RESTAURANT">RESTAURANT (매장관리)</option>
                  <option value="REVIEW">REVIEW (리뷰/댓글)</option>
                  <option value="NOTICE">NOTICE (공지사항)</option>
                  <option value="POPUP">POPUP (팝업/광고)</option>
                </select>
              </div>
            </div>
            ...
            <div className="flex gap-2 px-6 py-4 border-t border-gray-100">
              <button
                ...
                className={`flex-1 py-2 text-sm rounded-lg transition-colors
                  ${selectedMember ? "text-white bg-orange-600 hover:bg-orange-700" : "text-gray-300 bg-gray-100 cursor-not-allowed"}`}
              >
                등록 완료
              </button>
              <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white w-full max-w-sm mx-4 shadow-lg rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              ...
              <button onClick={() => setIsRoleModalOpen(false)} className="text-gray-300 hover:text-gray-500 transition-colors">✕</button>
            </div>
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="flex items-center border-b border-gray-50 pb-3">
                ...
                <span className={`px-2 py-0.5 text-xs rounded-full font-medium ${selectedAdmin.adminRole === "SUPER" ? "bg-purple-100 text-purple-700" : "bg-gray-100 text-gray-600"}`}>
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className={`flex-1 py-2.5 text-sm rounded-lg border transition-colors font-medium
                      ${newRole === "MANAGER" ? "bg-gray-700 text-white border-gray-700" : "bg-white text-gray-500 border-gray-200 hover:border-gray-400"}`}
                  >
                    MANAGER
                  </button>
                  <button
                    ...
                    className={`flex-1 py-2.5 text-sm rounded-lg border transition-colors font-medium
                      ${newRole === "SUPER" ? "bg-purple-600 text-white border-purple-600" : "bg-white text-gray-500 border-gray-200 hover:border-purple-400"}`}
                  >
                    SUPER
                  </button>
                </div>
            ...
            <div className="flex gap-2 px-6 py-4 border-t border-gray-100">
              <button
                ...
                className={`flex-1 py-2 text-sm rounded-lg transition-colors
                  ${(selectedAdmin.adminRole !== newRole || selectedAdmin.adminPart !== newPart) ? "text-white bg-orange-600 hover:bg-orange-700" : "text-gray-300 bg-gray-100 cursor-not-allowed"}`}
              >
                변경 완료
              </button>
              <button onClick={() => setIsRoleModalOpen(false)} className="flex-1 py-2 text-sm text-gray-400 border border-gray-200 hover:bg-gray-50 rounded-lg transition-colors">
                취소
              </button>
            </div>
          </div>
